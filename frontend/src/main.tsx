...
import "./styles.css";
import { api, demoReadyToFile, RtiDetail } from "./services/api";
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <div className="appShell">
      <aside>
        <Link className="brand" to="/dashboard">RTI Navigator</Link>
        <span>Track B lifecycle prototype</span>
        <nav>
          <Link to="/fixture">Ready-to-File Fixture</Link>
...
function Fixture() {
  ...
  async function begin() {
    setError("");
    try {
      const created = await api("/rtis", { method: "POST", body: demoReadyToFile });
      navigate(`/filing/${created.rti_id}/applicant`);
    } catch (err) {
      setError(String(err));
    }
  }
  return (
    <Page title="Validated Ready-to-File RTI" eyebrow="Track A fixture">
      <section className="band">
        <p>{demoReadyToFile.original_query}</p>
        <div className="requestBox">{demoReadyToFile.request_text}</div>
        <button onClick={begin}><Send size={16} /> File this RTI</button>
  ...
  return (
    <Page title="Applicant Details" eyebrow="Filing step 1">
      ...
      <section className="band">
        ...
        <Context rti={rti} />
        <div className="grid3">
          {Object.entries(form).map(([key, value]) => <input key={key} value={value} onChange={(event) => setForm({ ...form, [key]: event.target.value })} placeholder={key} />)}
        </div>
...
function Documents() {
  ...
  return (
    <Page title="Documents" eyebrow="Filing step 2">
      ...
      <section className="band">
        <Context rti={rti} />
        <button onClick={upload}><Upload size={16} /> Add demo document</button>
        <button className="secondary" onClick={() => navigate(`/filing/${rtiId}/payment`)}>Continue</button>
  ...
  return (
    <Page title="Demo Payment" eyebrow="Filing step 3">
      ...
      <section className="band">
        ...
        <button onClick={() => pay("SUCCESS")}><CreditCard size={16} /> Mark payment success</button>
        <button className="secondary" onClick={() => pay("FAILED")}>Simulate failure</button>
      </section>
    </Page>
  );
}
  ...
  return (
    <Page title="Review RTI" eyebrow="Filing step 4">
      ...
      <section className="band">
        ...
        <button onClick={submit}><Send size={16} /> Simulate submission</button>
      </section>
    </Page>
  );
}
  ...
  return (
    <Page title="Submission Recorded" eyebrow="Prototype confirmation">
      <section className="band">
        ...
        <Link className="buttonLink" to="/dashboard">Go to dashboard</Link>
      </section>
    </Page>
  );
}
...
function Dashboard() {
  ...
  return (
    <Page title="Dashboard" eyebrow="Citizen RTIs">
      <section className="toolbar"><button onClick={reset}><RefreshCw size={16} /> Demo reset</button></section>
      <section className="list">
        {rtis.map((rti) => (
          <article key={rti.id} onClick={() => navigate(`/rtis/${rti.id}`)}>
            <b>{rti.registration_number ?? "Not submitted yet"}</b>
            <span>{rti.authority_name}</span>
            <p>{rti.subject}</p>
            <small>{rti.status} · {rti.next_action.title}</small>
          </article>
        ))}
      </section>
    </Page>
        ...
        <div className="timeline">
          {timeline.map((event) => (
            <div className="timelineEvent" key={event.id}>
              <Check size={16} />
              <div><b>{event.title}</b><p>{event.description}</p></div>
            </div>
          ))}
        </div>
      </section>
...
function Context({ rti }: { rti: RtiDetail | null }) {
  return <div className="context"><FileText size={18} /> <b>{rti?.authority_name ?? "Loading..."}</b><span>{rti?.status}</span></div>;
}
    ...
    <BrowserRouter>
      <Shell>
        <Routes>
          ...
          <Route path="/filing/:rtiId/applicant" element={<Applicant />} />
          <Route path="/filing/:rtiId/documents" element={<Documents />} />
          <Route path="/filing/:rtiId/payment" element={<Payment />} />
          <Route path="/filing/:rtiId/review" element={<Review />} />
          <Route path="/filing/:rtiId/submitted" element={<Submitted />} />
