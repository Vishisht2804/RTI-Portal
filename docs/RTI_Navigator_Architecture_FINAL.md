...
Backend: POST /api/v1/intent/analyze → AI extract → validate.
...
Input: intent JSON. Output: {is_rti, jurisdiction, explanation}. The jurisdiction field is the final deterministic decision produced by F2.
UI: verdict card, plain-language reasoning.
Backend: rules/rti_classifier.py + rules/jurisdiction.py, deterministic keyword/category match. `jurisdiction.py` produces the final Central/State decision; AI may provide only a `jurisdiction_hint`.
...
Input: synthetic applicant data, optional mock document, mock OTP, mock ₹10 payment.
...
OTP is deterministic and simulated; no external OTP provider is used.
Failure: payment fail → PAYMENT_FAILED state, retry option; never claim success falsely.
Mocked: OTP, payment, document storage, govt submission. Real: form logic, state transitions, DB writes. Document upload is P0 demo functionality but is the first P2 filing feature cut if the team falls behind.
...
Flow: GET /api/v1/rtis (list), GET /api/v1/rtis/{id} (detail+events).
...
**Must-have (P0):** F1–F8 core paths, demo 3 scenarios, mock payment/OTP/submission, dashboard, timeline, next-action. Document upload is P0 demo functionality but is explicitly cuttable before the core applicant→payment→review→submit path if P2 falls behind.
...
Req: `{"text": "..."}` Res: `{"is_rti":true,"category":"health","jurisdiction_hint":"central","summary":"...","missing_information":[]}`
