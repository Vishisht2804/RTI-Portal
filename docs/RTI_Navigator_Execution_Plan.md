...
| `POST /intent/analyze` | P1 |
| `POST /authorities/recommend` | P1 |
| `POST /drafts/generate` | P1 |
| `POST /drafts/validate` | P1 |
| `POST /rtis` (create from Ready-to-File) | P2 |
| `POST /rtis/{id}/payment` | P2 |
| `POST /rtis/{id}/submit` | P2 |
| `GET /rtis`, `GET /rtis/{id}` | P2 |
| `POST /documents` | P2 |
| `POST /demo/reset` | Joint (seeds both halves) |
| `users` table | Joint schema, P2 writes (applicant) |
| `authorities` table | P1 |
| `drafts` table | P1 |
| `rtis` table | P2 (owns writes), P1 reads jurisdiction/category fields only |
...
- `jurisdiction` (enum: "central"|"state", required).
...
Transport: P1's frontend calls `POST /rtis` with this object as body when user clicks "File this RTI." P2's backend validates required fields (422 if `validation_status != "ready"`) and creates the `rtis` row, returning `{"rti_id": ..., "status": "READY_TO_FILE"}`. Frontend then routes to `/filing/:rti_id`.
...
**Joint (AM):** Execute Integration Procedure (section 12) — replace P2's mocked Ready-to-File JSON with P1's real `POST /rtis` call from the actual Ready-to-File screen. Run the 3 demo scenarios end to end.
...
- **Stage 4 — Full MVP integration (Day 4):** P1's Ready-to-File screen calls P2's real `POST /rtis`. Full journey connected. Testable: e2e demo-scenario walkthrough, manually.
...
Day 4: the ONE actual cross-team wire-up — P1's Ready-to-File screen now calls the real `POST /rtis` instead of P2 reading a static fixture. This is the single integration point in the entire project; everything before it was independent.
...
|---|---|---|---|---|---|---|
| `POST /rtis` | Ready-to-File object (2.1) | `{"rti_id":101,"status":"READY_TO_FILE"}` | P2 | P2 reads static fixture file instead of calling this | P1's frontend calls it live | contract test: P1 posts frozen fixture, P2 asserts 201 + shape |
| `GET /rtis/{id}` | — | RTI + status_events | P2 | n/a (P2-only until dashboard) | used from Day 3 | unit test on P2 side |
| `POST /intent/analyze` | `{"text":...}` | intent JSON | P1 | frontend reads static JSON | real from Day 3 | unit test on P1 side |
| `POST /authorities/recommend` | category/entities | authority JSON | P1 | static fixture | real from Day 3 | unit test on P1 side |
| `POST /drafts/generate` | intent+authority | draft JSON | P1 | static fixture | real from Day 3 | unit test on P1 side |
| `POST /drafts/validate` | draft text | validation JSON | P1 | static fixture | real from Day 3 | unit test on P1 side |
| `POST /rtis/{id}/payment` | — | payment result | P2 | mocked from start (always mocked, no "real" version exists) | n/a | unit test |
| `POST /rtis/{id}/submit` | — | registration number | P2 | mocked from start | n/a | unit test |
| `POST /demo/reset` | — | 200 | Joint | n/a | built Day 6 | manual + e2e script |

First APIs implemented for real: `POST /intent/analyze` and `POST /rtis` — these gate everything else, prioritized Day 3–4.
...
**Recommendation:** both people access the database **only through their own services**, never raw cross-service queries. P1's services never query `rtis`/`status_events` directly; P2's services never query `drafts`/`authorities` directly. The only cross-boundary data flow is the Ready-to-File JSON object passed over HTTP (`POST /rtis`), not a shared DB read. This avoids migration conflicts and keeps the "modular monolith" boundary real even though it's one codebase and one database — for a 2-person hackathon this is the safest way to avoid one person's schema change silently breaking the other's queries.

---

## 9. Frontend Integration (Ready to File → Applicant Details)

- **Routing:** P1's Ready-to-File screen is `/draft/:draft_id/ready`. On "File this RTI" click, frontend calls `POST /rtis` with the Ready-to-File object. On success, router pushes to `/filing/:rti_id/applicant` (P2's first screen).
- **State transfer:** no client-side state passed between the two screens beyond the `rti_id` in the URL — P2's screen always re-fetches `GET /rtis/{id}` on mount rather than relying on in-memory state from P1's screen. This keeps the boundary clean and refresh-safe.
- **Persistence:** the `POST /rtis` call is what actually persists the object server-side; nothing is held only in frontend memory across the navigation.
- **Refresh behavior:** if the user refreshes on `/filing/:rti_id/applicant`, the screen re-fetches from `GET /rtis/{id}` and continues — no data loss.
...
- **Error handling:** if `POST /rtis` fails (422 contract mismatch, 500 server error), P1's screen stays on Ready-to-File and shows the error inline — it never navigates forward on failure.
...
3. Connect P1's Ready-to-File screen to call real `POST /rtis` instead of P2's static fixture.
4. Run happy path — demo scenario 1 (ideal journey) end to end, both watching.
5. Run failure cases — demo scenario 2 (wrong jurisdiction, should never reach `POST /rtis`), demo scenario 3 (bad request reformulation), plus a deliberate payment failure.
...
**P2 one day behind:** cut document upload UI (assume no attachments for demo), cut additional-fee state (skip straight from AWAITING_RESPONSE to RESPONSE_RECEIVED in demo scenario).
...
- API/contract tests (Ready-to-File shape, `POST /rtis`) — **Joint**, must exist by Day 4 (written the morning of integration, section 12 step 2–3).
...
**Handoff point in the live demo:** at the moment P1 clicks "File this RTI" on the Ready-to-File screen — narration passes from P1 to P2 at exactly that click, mirroring the real API handoff at `POST /rtis`.
